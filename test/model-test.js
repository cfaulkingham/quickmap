...
const openCmd = Model.browserCommand(Model.openUrl({ lat: 1, lon: 2 }, null, null, "lookup"))
assert.deepStrictEqual(openCmd[0], "/usr/bin/omarchy-launch-browser")
assert.strictEqual(openCmd.indexOf("--"), -1)
assert.ok(openCmd[1].indexOf("https://www.openstreetmap.org/") === 0)
assert.deepStrictEqual(Model.browserCommand(""), [])
assert.deepStrictEqual(Model.browserCommand("https://evil.example/"), [])
assert.deepStrictEqual(Model.browserCommand("https://www.openstreetmap.org.evil.example/"), [])
